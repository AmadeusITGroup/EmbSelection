...
def main():
    ...
    use_cases = [
        ...
        'A retail company wants to analyze customer feedback from various sources like social media, product reviews, and surveys to understand the overall sentiment towards their products and services. By using text embeddings, they can convert customer feedback into numerical vectors and apply machine learning algorithms to classify sentiments as positive, negative, or neutral. Languages needed: Spanish, English'  
        'We need to power a product search and recommendation system over a catalog of 1 million items. Queries come from users typing short English phrases (up to 32 tokens), and documents consist of title and description (up to 128 tokens). The workflow requires fast Retrieval of topK candidates, followed by lightweight Reranking. The system must run on a single 4 GB GPU instance, use only open-source components under an Apache-2.0compatible license.', 
